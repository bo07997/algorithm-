...
<div style="width:1024px;" >
   ...
   <input id="title" type="text" class="form-control"   value="${update_blog.bTitle }" placeholder="文章标题" name="bTitle" style="width:512px ">
   </div>
   <div class="radioBox ">
    <script id="editor" type="text/plain" style="width:1024px;height:500px;">${update_blog.content }</script>
</div>
...
<div class="radioBox ">
<c:forEach var="updateBlog_categorie"  items="${updateBlog_categories}" >

<c:if test="${updateBlog_categorie.cId==update_blog.cateId}"> 
<input type="radio" name="catebyUser"  checked="true" id="my_${updateBlog_categorie.cId }" value="${updateBlog_categorie.cId }"><label for="radChl1">${updateBlog_categorie.cateName }</label>
     </c:if>
     <c:if test="${updateBlog_categorie.cId!=update_blog.cateId}"> 
<input type="radio" name="catebyUser"  id="my_${updateBlog_categorie.cId }" value="${updateBlog_categorie.cId }"><label for="radChl1">${updateBlog_categorie.cateName }</label>
     </c:if>
       </c:forEach>
</div>
</div>
...
<div >
  <div >
    ...
    margin-left: 15px;margin-bottom: 25px;  "  >${update_blog.introduction }</textarea>
    </div>

<div class="btn-group" data-toggle="buttons"  style="width:255px;margin-left: 355px;" >
 <button id="submit_blog"  onclick="update_blog(${loginuser.uId})" type="button" class="btn btn-primary" >提交更改</button>
...
<script type="text/javascript">
    ...
    function update_blog(uId) {
    		...
    		if($("#introduction_blog").val()==null||$("#introduction_blog").val().trim()==""){
          	...
          	$.ajax({
          		type:"POST",
          		url:"${pageContext.request.contextPath}/Blog/updateBlogByUser",
          		data:JSON.stringify({
          			"bTitle": $("#title").val(),
          		...
          		success:function(blog){
          			//打开新的页面
          			window.location.href ="${pageContext.request.contextPath}/Message/manager/"+blog.uId+"/1_0";
    ...
    $(document).ready(function(){
    	...
    	$.ajax({
      		type:"POST",
      		url:"${pageContext.request.contextPath}/Blog/getCSDNradio",
      		data:JSON.stringify({
      		...
      		dataType:"json",
      		success:function(blog){
      			//$("input[name='radChl']").value(blog.cateCsdn).attr("checked","checked");
      			$("input[name='radChl']").each(function()
      			{
      			   if($(this).val()==blog.cateCsdn)
      			{
