	...
	$.fn.zyComment = function(options,param){
		...
		return this.each(function(){
			...
			this.createAgoCommentHtml = function(){
				...
				html += '	<div class="text" style="font-size:2rem;padding-bottom:10px;border-bottom: 1px solid #DFDFDF;"> 评论区 </div>';
				html += '</div>';
				$(self).append(html);
				...
				$.each(para.agoComment, function(k, v){
					...
					item += '	<a class="avatar">';
					item += '		<img src="/ssm_project'+v.head+'">';
					item += '	</a>';
			...
			this.addNewComment = function(param){
				...
				item += '	<a class="avatar">';
				item += '		<img src="/ssm_project'+param.head+'">';
				item += '	</a>';
				item += '	<div class="content '+topStyle+'">';
